...
2026-09-20：上一批 300 次运行及 25 组质量预热通过，但完成后借核在 256 进程退化。
五次自然运行的最慢计算进程均为 155；现有公开汇总缺少该进程自己的生成、前沿、域内修复等计时，不能用各阶段跨进程最大值推断原因。

本次仅改分析器，直接读取集群上保留的 `rank_profiles.jsonl(.gz)`（逐进程原始指标），不需要重编译或申请计算作业。在仓库根目录执行：
...
整个批次的所有规模、策略会自动遍历，不受当前环境中的策略筛选影响。此次重分析只新增/更新诊断报告，不改原性能汇总、质量结果、SUCCESS（成功标记）、网格或原始指标；缺失/损坏/仍在运行的记录会明确报错并继续其他记录。

优先上传并查看每个规模的 `p*/CRITICAL_PATH_SUMMARY.txt`（关键进程摘要）。对应 `kernel_critical_summary.csv`（阶段表）和各策略 `analysis/kernel_critical_ranks.csv`（逐次明细）可用于进一步定位；它们会被正常纳入现有 Git（版本管理）结果上传。
明细只保留每次最慢计算进程、其同节点进程、生成/修复/优化及其子阶段的最慢进程，以及已有到达时间的最晚进程；不导出网格内容。

以后按原入口运行新实验，也会自动产生这些报告，原 `RESULT_SUMMARY.txt` 会包含关键进程摘要。当前无需为了补报告重新执行下面的 300 次实验命令。
完整方法、证据边界和本地检查见 [关键进程定位协议](../docs/experiments/20260920_critical_rank_diagnosis.md)。

## 已运行的收尾评价：贡献消融、规模扩展与体网格审计

研究分支 `agent/validate-node-affinity-pool`。主线冻结为 `node_elastic`（完成后借核）。最新批次中，限制提前借核的收益小且不稳定，不再继续叠加分配规则；历史策略仍可显式运行。
在仓库根目录使用原入口，构建成功后一次提交：
...
默认不运行模型。五组使用相同节点分配、CPU 池及初始核数：

| 组 | 面交换 | 内核策略 | 相邻组比较的贡献 |
|---|---|---|---|
| A | 原全收集 `baseline` | `node_original`，原串行修复 | 同资源原流程对照 |
| B | 稀疏 `sparse` | `node_original` | 稀疏通信 |
| C | 稀疏 | `node_native`，并行修复 | 修复并行化 |
| D | 稀疏 | `node_fixed`，固定资源整段接口 | 整段线程组复用及资源接口的净贡献 |
| E | 稀疏 | `node_elastic`，完成后借核 | 节点内动态资源分配 |

`node_original` 使用现有 schedule=0 内核，并不是全程序单线程；生成与优化仍使用同样的线程预算。
D/C 的差异包含资源接口开销，不能把全部收益单独归给线程启动次数。

默认 **4/16/64/128/256 进程，对应 1/4/16/32/64 节点**；每节点 4 进程、16 个物理核，初始每进程 4 核。
表面细化 2、体细化 1；natural/split（自然计时/等待执行拆分）各预热 1 次、正式 5 次。
共 **300 次运行、250 次正式测量**，其中 25 次自然预热同时完成全量局部体网格审计，不额外启动验证运行。
不同规模分别提交；同规模轮换策略顺序。两个原修复通信组在同一策略轮次内交替先后，其余组依次轮换；不是五组完全随机顺序。
`run_node_affinity_validation.sh` 保留同样组合，原文件名不变。

`QUALITY_WARMUP=1` 要求有自然计时和至少一次预热；只有 natural/repeat_0 接收 `--validate-volume`，稀疏组同时执行 `--verify-faces` 的全收集参考核对。
审计发生在体细化之后、幽灵单元插入之前，遍历全部本地拥有的四面体，记录：

- 非法索引、非有限坐标、重复顶点、零体积、混合方向；面非流形、同侧连接、缺失/孤立表面和零面积表面。
- 平均比形状质量（正四面体为 1）、十区间分布、极值和平均值；六个内部二面角的极值；绝对体积总和。
- 按坐标构造、与局部编号无关的表面和体单元指纹；重复表面与内部标记面单独统计，允许合法双侧材料界面。

结构错误使该预热失败，保留诊断并继续余下运行。相邻消融组还比较边界指纹、计数、体积及质量不退化；缺失审计不能报告通过。
完整判据见 [收尾协议](../docs/experiments/20260915_final_evaluation.md)。这是预热网格的全量局部审计，**不覆盖任意非相邻单元相交、CAD 距离、幽灵层或 Elmer 输出，也不等于逐次验证全部正式运行的几何**。
审计会增加预热耗时和内存；正式运行不执行这些扫描，预热时间不进入正式中位数。

各规模优先看 `p*/RESULT_SUMMARY.txt`，并保留：

- `ablation_summary.csv`：四项相邻贡献及 A→E 总收益，含同编号重复胜出次数、质量门槛；natural 用于净收益，split 用于等待归因。
- `mesh_quality_summary.csv`：五组预热审计；对应 `repeat_0/quality_summary.json` 保留逐进程计数和指纹。
- 原 `kernel_summary.csv`、`kernel_phase_summary.csv` 及逐进程时间线：资源与修复诊断。

`COMMUNICATION_ABLATION=1` 只为 `node_original` 自动增加 baseline，其他策略沿用 `ALGORITHMS`。
显式复现历史组合时可设置 `QUALITY_WARMUP=0 COMMUNICATION_ABLATION=0`；报告将注明未要求质量审计。
更改配置或二进制必须使用新结果目录。各规模产生的单元总数可能不同，跨规模曲线必须同时报告实际工作量，不称为严格固定网格强扩展。
本地已验证几何计算、公共接口编译与驱动的成功/失败路径；完整 MPI/OCC 构建和真实多节点结果仍以本次集群运行验证为准。

当前仓库：`1482908163/NETGEN`；当前分支：`agent/validate-node-affinity-pool`。历史数据和结论保留原来源。
...
本轮默认配置和命令以上方“收尾评价”为准；主线保留稀疏通信、并行修复与完成后借核。
旧策略仅显式选择时运行。历史结果和原子借核方案见 [上一轮记录](../docs/experiments/20260913_capacity_driven.md)。
...
默认父驱动阶段为 `kernel`，各策略子驱动使用 `communication`，方法为 `none`。程序使用 `--communication-only`，绕过任务构造、均衡特征与修正、节点映射和模型统计分发。稀疏协议本身不变。`balance/combined` 在通信阶段会被拒绝，防止旧环境变量意外重新启用失败方案。

历史四组任务对照仅供复现：需同时显式选择 `evaluation` 与 `task_queue`；其 baseline 是 P−1 工作者的静态任务版本，与当前 baseline 含义不同。历史入口不作为推荐方案。

## 构建与小规模正确性检查

以下为独立小规模开发检查；本轮收尾实验只需使用页首命令。运行原名称的构建脚本。脚本使用当前目录，不会跳回硬编码的另一个检出目录。已有默认安装目录名称保持不变。

```bash
git fetch origin
git switch agent/validate-node-affinity-pool
bash build_project.sh
...
`--verify-faces` 会实际运行原全收集作为参考，逐项比较面记录及共享顶点/边/面的进程集合；因此不进入正式性能轮。独立检查仍不采集性能；仅本轮 natural/repeat_0 的体网格审计允许同时记录预热诊断。该检查通过后仍需比较完整输出。均衡方案另检查几何边界、单元规模和网格质量。
...
历史大规模通信配置需显式设置 `EXPERIMENT_PRESET=production`：1024、2048、4096、8192进程，对应64、128、256、512节点，L=R=3。
若需要先预检，在配置区改为 `pilot`：16、32、64进程，对应1、2、4节点，L=R=1，并执行稀疏面正确性核对。

以下表格描述历史 production 通信对照。本轮默认 cooperate/kernel，以页首五组配置为准，不读取校准目录、不拟合模型。

| 设置 | 历史 production 通信基线 |
|---|---|
