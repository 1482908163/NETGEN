...
研究分支为 `agent/validate-node-affinity-pool`。本轮同时修改 Netgen（网格库）和应用，使用原入口完整重建，然后一次提交：
...
`run_node_affinity_validation.sh` 仍可作为同一套配置的便捷入口。默认暂不运行模型，比较四组：

| 策略 | 借核来源 | 归还方式 |
|---|---|---|
| `node_fixed`（固定分配） | 不借核 | 不涉及 |
| `node_elastic`（完成后借核） | 仅已完成进程 | 租约结束 |
| `node_stage`（阶段借核） | 另包括未完成进程的串行窗口 | 租约结束 |
| `node_reclaim`（安全点归还） | 同阶段借核 | 原所有者请求后，在完整网格操作之间归还 |

新策略在串行阶段保留一个主核，其余核可被同节点忙进程借用。恢复并行前必须收回完整初始保底核，默认至少 4 核；等待者只运行主核。
归还先停止工作线程、后释放核，不在网格操作中途抢占。分区、网格归属和操作顺序不变。
`node_reclaim` 另在 Delaunay（德劳内）内部优化轮次间设置检查点，保留原十轮操作和环境更新。

默认 1/4/16 节点，对应 4/16/64 进程；每节点 4 进程、16 核。表面细化 2、体细化 1。
四策略 × 三规模 × natural/split（自然计时/等待执行拆分）×（预热 1 次＋正式 5 次），共 **144 次运行、120 次正式结果**。
不同规模并行提交、统一延时；同一作业内轮换策略顺序。失败继续，单次超时 600 秒，保留失败诊断；成功后按原规则清理中间网格。
常用参数仍在原脚本顶部修改，实际网格规模以输出为准。

先看各 `p4/p16/p64/RESULT_SUMMARY.txt`（结果摘要），细节见：

- `kernel_summary.csv`（策略比较）：自然核心时间、等待、质量计数、启停成本，以及相对固定和完成后借核的加速比。
- `kernel_phase_summary.csv`（阶段租约）：最少/最多线程、低于保底核次数、租约量。
- 各策略 `analysis/node_resource_timeline.csv`（逐进程资源记录）：借核时刻、提前借核量、归还与等待。
- `node_coop_diagnostics.txt`（节点诊断）：核池和各进程初始核列表。

新增阶段闲置核秒与提前借核核秒，用于区分“存在串行窗口”和“实际借出资源”；它们是资源状态积分，不代表硬件忙碌率。
收回核等待与线程组启停已经包含在核心时间中，不得再相加。只在同一节点比较资源时间戳。
阶段重建允许增核、缩核或不变，三类合计必须覆盖全部重建；旧原子尾部模式仍要求实际增核。

判据：先检查线程资源和网格计数，再看提前借核是否发生、归还等待是否抵消收益，最后以自然核心时间判断净改善。
正常退出、单元数一致及非法标记未增加不能替代完整几何质量验证。本轮最终提交仍待完整集群编译和实测。
协议与验证范围见 [阶段借核研究记录](../docs/experiments/20260914_stage_lending.md)。
