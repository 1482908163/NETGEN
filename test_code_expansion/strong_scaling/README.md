...
研究分支 `agent/validate-node-affinity-pool`。本轮限制提前借核的阶段和次数，沿用已运行的内核接口。
在仓库根目录使用原入口，构建成功后再提交：
...
| 策略 | 未完成来源的提前借核 | 检验目的 |
|---|---|---|
| `node_fixed`（固定） | 不借核 | 固定资源基线 |
| `node_elastic`（完成后借核） | 不允许 | 上一版有效基线 |
| `node_reclaim`（全阶段归还） | 原阶段均可 | 上一轮方案对照 |
| `node_selective`（最终优化借核） | 仅最终优化，可多次 | 限制阶段是否减少代价 |
| `node_once`（单次提前借核） | 仅最终优化，每进程最多一次 | 限制扩缩是否改善净收益 |

两种新策略的生成与修复阶段仅借已完成进程的核。最终优化可借串行中的未完成进程核，原所有者请求归还时仍优先响应。
单次策略在持有提前租约时暂停额外增核，保持该租约；一旦归还或正常结束，不再申请第二次提前租约。
已完成来源的常规借核仍可多次发生。每次进入并行阶段前收回完整初始保底核，默认至少 4 核。
资源归还保持“先停止工作线程，后释放核”。新策略不启用生成内部的额外安全点，不修改网格操作顺序、分区或通信算法。

默认 4/16/64 进程，对应 1/4/16 节点；每节点 4 进程、16 核，初始每进程 4 核。
表面细化 2、体细化 1；natural/split（自然计时/等待执行拆分）各预热 1 次、正式 5 次。
五策略三规模共 **180 次运行、150 次正式结果**。各规模并行提交、统一延时、轮换策略顺序，失败继续及清理规则保留。
`run_node_affinity_validation.sh` 保留相同默认组合，参数仍可在原脚本顶部修改。

先看各 `p4/p16/p64/RESULT_SUMMARY.txt`（摘要）。原 `kernel_summary.csv`（策略表）增加相对两种新对照的比较、
同重复编号胜出次数和耗时下降百分比中位数；逐轮胜出只用于观察波动，不代表统计显著性。
`kernel_phase_summary.csv`（阶段表）增加提前借核和归还次数，分析器检查两种新策略这些事件只出现在最终优化，单次策略每进程不超过一次。
逐进程资源记录增加暂停增核计数，所有者归还请求不能被暂停规则延后。

上一轮 144 次全部正常退出，但全阶段安全点归还相对完成后借核，在 4/16/64 进程自然时间仅变化 -0.30%/-2.29%/+0.44%。
整段结束归还明显退化，已退出默认组合。见 [上一轮结论及本轮协议](../docs/experiments/20260915_selective_lending.md)。

判据：先通过资源及网格计数检查，再看线程组重建、收回等待与自然核心时间。
若限制阶段或次数没有稳定收益，则保留完成后借核，停止无依据叠加分配规则。
资源核秒不是硬件忙碌率；等待和启停已包含在核心时间中，不重复相加；单元数及非法标记对照不能替代完整质量验证。
本轮执行环境不可用，最终版本尚未运行编译和集群实验，必须用原构建入口重建后验证。
...
本轮默认配置和命令以上方“阶段借核一次提交验证”为准；阶段借核保留稀疏通信与并行修复。
