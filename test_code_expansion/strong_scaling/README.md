...
在 `agent/validate-node-affinity-pool` 验证分支、`strong_scaling` 目录执行：

```bash
./run_node_affinity_validation.sh
```

默认申请1节点、4进程、每进程预留4个物理核，共16核。沿用当前小规模输入和细化参数，
比较 `repair`（原并行修复基准）、`node_fixed`（固定资源接口）、`node_lend`（普通空闲借核）、
`node_model`（模型指导分配）。每种策略分别运行 `natural`（自然计时）和 `split`（等待/执行拆分），
各预热1次、正式3次，共32次运行，其中24次进入正式汇总。需要修改子集或规模时，直接编辑此入口配置区。

同一作业内按轮次交替策略，避免4个策略同时争用同一份16核资源。指定多个进程规模时，
各规模作业仍并行提交并采用统一最早启动时间。普通失败或单次超时会记录状态并继续其他运行；
默认每次600秒，超时终止后30秒仍未退出则强制终止。整个作业被集群取消或节点故障时无法保证继续。

原修复基准由 Slurm（作业调度系统）按核绑定；协作模式关闭逐进程绑定，由节点资源接口划分共享核池。
预热不参与正式中位数；不同计时模式分别比较。成功运行自动清理已知中间网格并无损压缩记录，失败现场保留。

结束后优先查看同一结果根目录下 `p4/RESULT_SUMMARY.txt`（结果总览）和 `p4/kernel_summary.csv`（策略比较表）。
汇总包含退出完整性、自然核心时间、拆分等待、借核量、峰值线程数、模型采用次数及质量计数对照。
固定资源模式与原修复基准比较，借核模式与固定资源模式比较。没有发生借核或模型未被采用时会明确提示；
程序正常结束和计数对照通过均不代表完整网格质量已证明。

本次套件调整只涉及脚本与分析器；已编译过 `66272c2` 修复版本时无需重新编译。
