...
默认1节点、4进程、每进程初始4核，共16核。使用相同节点核池、相同初始核划分进行六组消融：

| 策略 | 验证目的 |
|---|---|
| `node_native`（同核布局原修复） | 初始化相同核池后直接调用原修复接口，不执行阶段租约 |
| `node_scoped`（旧细粒度固定接口） | 保留上一轮固定接口，检验反复创建线程组的影响 |
| `node_fixed`（整段修复固定接口） | 整次修复持有4核并复用线程组，与同核原修复比较 |
| `node_lend`（无保底贪心） | 整段修复租约，保留先到者领取空闲核的策略 |
| `node_guarded`（保底借核） | 未完成进程保留4核，只借已完成进程释放的核 |
| `node_model`（保底模型分配） | 与保底借核使用相同资源约束，额外用阶段模型选择核数 |

所有策略启用同样的并行修复。`node_fixed/node_lend/node_model` 的内部策略相对上一轮已有变化，
通过版本元数据 `node_coop_v2` 区分；不要跨版本混合重复样本。旧结果原样保留。

每组分别运行 `natural`（自然计时）和 `split`（等待/执行拆分），各预热1次、正式3次，
共48次运行，其中36次进入正式汇总。默认仍是小规模输入，实际体单元数以本批输出为准。
...
借核只在阶段边界生效，已开始的阶段不抢占；小规模没有借核或模型没有被采用时，汇总会明确说明。
本轮假设、对照关系与文献出处见 [节点资源第二轮研究记录](../docs/experiments/20260912_node_resource_v2.md)。
