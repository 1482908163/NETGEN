...
本轮验证分支为 `agent/validate-node-affinity-pool`。本次修改了应用侧节点调度器，必须重新编译应用；仍使用原构建入口，
在仓库根目录执行：
...
cd strong_scaling
./run_node_affinity_validation.sh
```

当前主线验证原子预留与持续领取，暂不运行模型。默认同时提交1/4/16节点，对应4/16/64进程；
每节点4进程、16核，每个未完成进程保留4核。每个规模比较以下四组：

| 策略 | 验证目的 |
|---|---|
| `node_fixed`（整段固定） | 基础4核，保留线程组 |
| `node_tail`（安全点借核） | 修复轮次、最终优化操作边界响应已完成进程释放的核 |
| `node_reserved`（原子预留） | 保留完成事件触发方式；重建前预留新核和已有借入核，保证实际增核 |
| `node_elastic`（持续领取） | 相同原子预留，但每个安全点检查容量，接收阶段归还的核 |

四组启用同样的并行修复和操作顺序。两组新策略使用 `node_atomic_v1` 元数据，
保留修复及最终优化阶段借核，允许同一进程多次增核；每次重建必须严格增加核数。
`node_reserved` 对照 `node_tail` 检验预留作用，`node_elastic` 对照 `node_reserved` 检验容量触发作用。
旧受限申请和剩余工作优先策略保留，仅显式选择时运行。

每组分别运行 `natural`（自然计时）和 `split`（等待/执行拆分），各预热1次、正式3次，
每规模32次运行，三规模共96次，其中72次进入正式汇总。
本轮所有对照组统一使用 LEVELS=2、REFINES=1，与 20260913-222013 批次保持一致。
实际体单元数以输出为准，不能把与上一批的绝对耗时变化当作策略收益。
参数仍集中在 `run_node_affinity_validation.sh` 顶部；不同规模作业并行提交、统一启动延时，
同一作业内轮换策略顺序。失败继续；单次600秒超时，30秒后仍未退出则强制终止。
成功运行清理已知中间网格并压缩原始指标，失败现场保留。

结束后先看本批各 `p4/p16/p64` 中的 `RESULT_SUMMARY.txt`（结果总览），需要细节再看：

- `p4/kernel_summary.csv`（策略比较）：核心耗时、拆分等待、质量计数、线程组启停和相对各对照的加速比。
- `p4/kernel_phase_summary.csv`（阶段租约）：阶段租约次数、最少/最多线程、低于初始核数的次数及租约核秒。
- 各策略 `p*/analysis/node_resource_timeline.csv`（节点资源时间记录）：逐进程准备/完成、首次/末次借核时刻及借核量；只在同一节点内比较时间戳。
- 各次 `node_coop_diagnostics.txt`（节点诊断）：节点核池及每个本地进程的初始核列表，用于核对同核布局。

线程组启停耗时已包含在内核时间中；不同子计时可能嵌套，不得再次加到总时间。
阶段秒为各进程租约时长之和，不是关键路径。正常结束、单元数一致和标记数未增加均不代替完整质量验证。
检查可借核闲置核秒、预留过渡核秒、实际增核次数、节点完成跨度，以及自然核心时间和最大计算时间。
只有实际增核且整体核心耗时下降，才能支持负载均衡收益。没有增核时汇总会明确说明。
本轮边界和判据见 [容量驱动借核研究记录](../docs/experiments/20260913_capacity_driven.md)。
...
主线为容量驱动的原子借核，保留稀疏通信及并行修复；模型和旧策略仅供显式复现。见 [本轮实现说明](../docs/experiments/20260913_capacity_driven.md)。

**需要重新编译应用，原构建脚本会处理依赖。** 在研究分支的仓库根目录依次执行，构建成功后再提交：
...
默认 cooperate（节点协作）：1/4/16节点，对应4/16/64进程；每节点4进程、固定16核，初始每进程4线程。对比 node_fixed（整段固定）、node_tail（原安全点）、node_reserved（原子预留）、node_elastic（持续领取）。全部使用稀疏通信，分别运行自然计时和等待拆分；1次预热、3次正式，共96次运行。LEVELS=2、REFINES=1，实际规模以输出为准。

仍只在 run_experiments.sh 顶部修改参数，不需要命令行传一串变量。未完成进程保留4核，已完成进程保留1个主核，忙进程最多使用同节点13核。集群若不允许作业内跨进程核绑定，程序会明确失败。

结果先看各 p4/p16/p64 下的 RESULT_SUMMARY.txt（结果摘要）和 kernel_summary.csv（内核汇总）。摘要给出相对固定、原安全点和原子预留策略的比较，以及实际增核、闲置核秒和节点完成跨度。原压缩、清理和续跑规则保留，使用新结果目录。代码可运行不代表已证明性能收益或创新性，需以同资源实验判断。
