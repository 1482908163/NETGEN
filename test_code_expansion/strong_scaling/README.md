...
当前主线验证安全点借核，暂不运行模型。默认同时提交1/4/16节点，对应4/16/64进程；
每节点4进程、16核，每个未完成进程保留4核。每个规模比较以下三组：

| 策略 | 验证目的 |
|---|---|
| `node_fixed`（整段固定） | 基础4核，保留线程组 |
| `node_guarded`（阶段边界借核） | 原保底策略，仅在整段开始时申请额外核 |
| `node_tail`（安全点借核） | 修复轮次、最终优化操作边界响应已完成进程释放的核 |

三组启用同样的并行修复、相同阶段内操作顺序。新策略增加 `node_tail_v1` 元数据，
仅在出现新释放核时重建线程组，没有释放事件则保持原线程组。

每组分别运行 `natural`（自然计时）和 `split`（等待/执行拆分），各预热1次、正式3次，
每规模24次运行，三规模共72次，其中54次进入正式汇总。默认仍是小规模输入，实际体单元数以本批输出为准。
...
结束后只需先看本批 `p4/RESULT_SUMMARY.txt`（结果总览），需要细节再看：
...
检查“安全点检查次数、重建次数、实际增核次数”以及自然核心时间、最大计算时间和同步等待。
只有实际增核且整体核心耗时下降，才能支持负载均衡收益。没有增核时汇总会明确说明。
本轮边界和判据见 [安全点借核研究记录](../docs/experiments/20260913_tail_balance.md)。
...
主线为保底安全点借核，保留稀疏通信及并行修复；模型和旧贪心策略仅供显式复现。见 [本轮实现说明](../docs/experiments/20260913_tail_balance.md)。
...
默认 cooperate（节点协作）：1/4/16节点，对应4/16/64进程；每节点4进程、固定16核，初始每进程4线程。对比 node_fixed（整段固定）、node_guarded（阶段入口借核）、node_tail（安全点借核）。全部使用稀疏通信，分别运行自然计时和等待拆分；1次预热、3次正式，共72次运行。LEVELS=1、REFINES=1，实际单元数以输出为准，不是百亿规模测试。

仍只在 run_experiments.sh 顶部修改参数，不需要命令行传一串变量。未完成进程保留4核，已完成进程保留1个主核，忙进程最多使用同节点13核。集群若不允许作业内跨进程核绑定，程序会明确失败。

结果先看各 p4/p16/p64 下的 RESULT_SUMMARY.txt（结果摘要）和 kernel_summary.csv（内核汇总）。摘要给出相对固定和阶段入口策略的比较，以及安全点检查、重建、实际增核次数。原压缩、清理和续跑规则保留，使用新结果目录。代码可运行不代表已证明性能收益或创新性，需以同资源实验判断。
