...
20260922-233233 六路线已完成，但四份切分增加单元数并降低部分质量指标，动态路线仍慢于参考路径。
已修复细化子面的朝向字段未赋值问题；任务指纹包含该字段，因此需要重编译后重测。

本轮保留六路线、128/256 进程、质量预热、正常/拆分计时及三次正式重复。
WORKLET_FACTOR 默认改为 1：每个原分区一个任务，避免额外切面，先隔离纯调度与回传开销。
这不是已经证明的加速方案；单个任务内部仍不能跨进程分摊。
统一配置区可显式改回 4，以复核上一轮切分实验。
...
各规模作业并行申请、统一延时启动；路线交错运行，失败继续后续任务。
先看 p128/ROUTE_SUMMARY.txt、p256/ROUTE_SUMMARY.txt 及 route_comparisons.csv。
保留参考路径直接对照、质量门槛、所有权及任务指纹检查，不放宽门槛来接受失败结果。
本轮依据见 [完整对照分析及粒度调整](../docs/experiments/20260922_worklet_full_matrix.md)。
