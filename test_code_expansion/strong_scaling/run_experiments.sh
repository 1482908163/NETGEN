...
[[ "${EXPERIMENT_PRESET}" != cooperate ]] || { default_kernel_counts=4;default_kernel_schedulers="node_fixed node_tail node_budget node_priority"; }
...
if [[ "${EXPERIMENT_STAGE}" == kernel ]]; then
    ...
    for scheduler in ${KERNEL_SCHEDULERS}; do
        [[ "$scheduler" == static || "$scheduler" == cavity || "$scheduler" == repair || "$scheduler" == frontier || "$scheduler" == node_native || "$scheduler" == node_scoped || "$scheduler" == node_guarded || "$scheduler" == node_fixed || "$scheduler" == node_lend || "$scheduler" == node_model || "$scheduler" == node_tail || "$scheduler" == node_budget || "$scheduler" == node_priority ]] || { echo "未知内核策略：${scheduler}" >&2;exit 2; }
    done
